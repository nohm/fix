class Stats < ActiveRecord::Base
  ...
  def self.generate_chart title, subtitle, data, extended
    if extended
      data = [
      ...
      Stats.generate_bar_chart([I18n.t('stats.status.new'),I18n.t('stats.status.tested'),I18n.t('stats.status.repaired'),I18n.t('stats.status.scrap'),I18n.t('stats.status.waiting'),I18n.t('stats.status.ready'),I18n.t('stats.status.sent')], 6, data)
    end
  end

  def self.generate_chart_status title, subtitle, data, extended
    if extended
      data = [
      ...
      Stats.generate_bar_chart([I18n.t('stats.status.tested'),I18n.t('stats.status.repaired'),I18n.t('stats.status.scrap'),I18n.t('stats.status.skipped')], 3, data)
    end
  end
  ...
  private
    ...
    def self.build_chart title, data
