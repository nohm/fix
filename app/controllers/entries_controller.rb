class EntriesController < ApplicationController
  ...
  def update
    ...
    if @entry.update(params[:entry].permit(:appliance_id,:number,:brand,:typenum,:serialnum,:defect,:ordered,:test,:repaired,:ready,:scrap,:accessoires,:sent,:note,:company))
      redirect_to entries_path, :page => Entry.page(params[:page]).total_pages, :notice => "Entry updated."
    else
      @appliance_names = Appliance.pluck(:name, :id)
      render 'edit'
    end
  ...
  def create
    ...
    if @entry.save
      redirect_to entries_path, :page => Entry.page(params[:page]).total_pages, :notice => "Entry added."
    else
      @appliance_names = Appliance.pluck(:name, :id)
      render 'new'
    end
