	...
	def index
		...
		@company.apptypes.each do |type|
			...
			type_entries.each do |entry|
				...
				status = Stats.process_status(entry)
				type_all[status] += 1
			...
			unless type_entries.length == 0
				...
				@charts[type.brand_type].append Stats.generate_chart(type.brand_type.gsub('_',''), type_entries.length, type_all, params[:type])
				unless type_min[0] + type_min[1] + type_min[2] + type_min[3] == 0
					@charts[type.brand_type].append Stats.generate_chart_status(type.brand_type.gsub('_',''), type_entries.length, type_min, params[:type])
				else
		...
		@charts[:global].append Stats.generate_chart(@company.short, @shipment.entries.length, type_global_all, params[:type])
		@charts[:global].append Stats.generate_chart_status(@company.short, @shipment.entries.length, type_global_min, params[:type])
	end
end
